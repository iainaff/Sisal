...
printable nodename[ifmaxnode + 1];
printable errvname[ifmaxerror + 1];
printable typename[ifmaxtype + 1];
printable basname[ifmaxbasic + 1];
printable graphname[19];
ndclasstype nodeclass[ifmaxnode + 1];   /* dlz - 8/87 */
FILE *source, *diag;
infilelist infile;
parrec *outfile;
/* these vars are needed by module using the graph */
stentry *typetable[entrymax];
/* */ int tthwm;   /* symbol table high water mark */
int univmodulecnt, univnodecnt, univedgecnt;
linkrec *funclist;
node *firstfunction, *module;
stryng stamp['[' - 'A'];
long stampset[9];
semanticrec *topsemstk;   /* Top of Semantic Stack */
namelistrec *definelist;
...
long arithtypes;   /* holds TT labels for int,real,doub */
long algebtypes;   /* holds TT labels for int,real,doub,bool */
long streamorarray;
/* holds TT code for stream and
                                                                       array types*/
namelistrec *includelist;
/* holds stack of names of files that have
                                   been included.  List is checked to avoid
                                   recursive includes*/
boolean typedefsfinished;
...
/*------------------- SYMBOL TABLE VARS ----------------*/
treestackobj *currentlevel;   /* pointer to current Lex Level */
/*------------------- DEBUGGER VARS -------------------*/
long breakpointset[maxsemanticnumber / 32 + 2];
...
parrec *xrefparam;
/*--------------------- XREF --------------------------*/
xrefrec idunsorted[1000];
int idsorted[1000];
int idxrefmax;
inputbuffer *linebuf, *oldline;
unchar tokenstart;
int linenumber, tokenlineno;
boolean endfile;
int parsestack[maxstack + 1];
Char stringspace[maxstring + 1];
boolean epsprod[maxprod + 1];
int stringptr;
SHORT stackptr;
SHORT oldtoken;
boolean echoed;
ptabrec *table[maxsym + 3];
stryng symtab[maxsym + 3];
stringrec symkluge[maxsym + 3];
int prodspace[maxprodspace + 1];
/* all the productions. (strings of symbols) */
prodrec productions[maxprod + 1];   /* pointers into prodspace*/
SHORT prodspceptr;
int numprods, numsymbols;
stryng inputtoken;
int toconsume, column;
/* !! variables for error correction */
int myinfinity, numterms;
rabrec *readaheadbuf;   /*head of buffer of inserted tokens*/
FILE *ptableout, *etableout;
/* index : intfile; */
FILE *inputfile;   /* file to be parsed */
boolean peeking;
int errorcount;
int deltriedhisto[maxhisto + 1];
    /* histograms of deletions and insertions */
int delmadehisto[maxhisto + 1], inshisto[maxhisto + 1];
int tokinserted, tokdeleted;
char insertspace[maxinsertspace + 1];
int deletecosts[maxterm + 1];
insertstringrec costtable[maxsym + 3];
pretabarray prefixtable;
int prefixindex[maxterm + 1];   /*sks*/
char curerrsym;   /* current terminal symbol resident in prefixtable*/
boolean errortables;
    /* whether error correction tables were created */
int insertptr, savinsertptr, numdeltried;
    /* number of deletions considered */
FILE *reservedwords;
/*---------- Current State of the editor ----------*/
boolean withininputmode, quittingtime;   /* time to leave the editor */
int tokenposition;   /* character position within a token */
/*--------- variables for prettyprinting-------------*/
SHORT tempptr;
boolean readingbinaryfile;   /* true only when reading a binary file */
unchar debugflags[32];
/*-------- variables for maintaining stringspace ---------*/
extstringspace *firststring;   /* pointer to the first extent */
extstringspace *curextent;   /* pointer to current Extent */
int curbase;
...
int singlechar['\177' - ' '];   /* token numbers */
int reuseindex;
Char reusestring[maxbackup + 1];   /* pos 0 not used */
int statehistory[maxbackup + 1];
long badcharset[9];
/*------- Super Record, where most of the files are ----------*/
tablerec p;
FILE *listfile;
boolean listing, xref;
int rea;
Char debugchar;
boolean debugcg, debug, cortrace, firsttime, done, commentflag,
	       includeflag;
includestackrec *topincludestk;
stryng curinputfile, newfname;
int newlinenum;
parrec *listingparam, *debugparam, *pass1param;
int semerrcnt;
parrec *listfparam;
stryng directoryname, defaultdirectoryname, filename;
parrec *directoryparam, *paramlist;
boolean passerr;
stryng listfilename, stampstr, versionstr, versionnum;
...
Void deletestring(s, first, last)
stryng *s;
char first;
int last;
{
...
boolean matchstrings(s1, s2, pos)
stryng *s1, *s2;
char pos;
{
  int i, ix;
...
*/
Void buildname(newname, dev, name, ext)
...
Void splitname(origname, dev, name, ext)
...
Void defaultfilename(name, defname)
stryng *name, *defname;
{
...
Void gettermline(termline, termtext)
...
Void setparameter(param, paramvalue)
parrec *param;
stryng *paramvalue;
{
  int pos;
...
Void getparamvalue(param, prompt)
parrec *param;
stryng prompt;
{
  stryng parvalue;
...
Void defaultext(name, oldname, ext)
stryng *name, *oldname;
Char *ext;
{
  stryng dev, nom;
...
Void askordefault(par, def, inter, prompt)
parrec *par;
stryng *def;
boolean inter;
Char *prompt;
...
Void readcommandline(infile, outfile, paramlist)
parrec **infile;
...
boolean openinputfile(filename, source)
stryng *filename;
FILE **source;
{
...
boolean openlistingfile(listfilename, list)
stryng listfilename;
FILE **list;
{  /* OpenOutputFile */
  stryng dev, name, ext;
...
boolean openuncompfile(uncompfilename, uncomp)
...
node *totagcasesubgraph(t, tagnum)
node *t;
int tagnum;
{
  assoclist *alist;
...
/* over port range      */
port *nextinputedgenextport(e, p)
port *e;
int p;
{
...
int largestfieldnumberofrecord(r)
stentry *r;
{
...
Void typeandnameofnthfield(r, n, ftype, fname)
stentry *r;
int n;
stentry **ftype;
stryng *fname;
...
Void typeandnameofnthtag(u, n, ttype, tname)
stentry *u;
int n;
stentry **ttype;
stryng *tname;
...
Void compacttypetable(tthwm, oldtthwm)
int *tthwm, oldtthwm;
{
...
stentry *makefuntypefromgraph(g)
node *g;
{
...
Void linkcallnodetofunction(cnode, fgraph)
node *cnode, *fgraph;
{
...
Void disconnectedgefromsource(e)
port *e;
{
  /*in out*/
  /* ----------------------------------------------------
...
Void insertnode(parentnode, prevnode, newnode)
...
Void changeedgedest(e, newdestnode, newdestport)
port *e;
node *newdestnode;
int newdestport;
{
...
Void changeedgesrc(e, newsrcnode, newsrcport)
port *e;
node *newsrcnode;
int newsrcport;
{
...
Void putnodebefore(n, intransit)
node *n, *intransit;
{
...
Void shiftinputports(n, startport, amount)
node *n;
...
Void shiftoutputports(n, startport, amount)
node *n;
int startport, amount;
{
...
Void linknodeintograph(n, parent, prev)
node *n, *parent, *prev;
{
...
Void replacecallwithgraph(cnode, fgraph)
node **cnode, *fgraph;
{
  /* Preconditions:
...
Void replacefunctiongraph(newfg, oldfg)
node *newfg, *oldfg;
{
...
Void nextbodyportname(level, str)
treestackobj *level;
stryng *str;
{
...
Void freeedgelistptr(edgelist_)
edgelist **edgelist_;
{
  Free(*edgelist_);
}
...
Void freereferencesptr(refptr_)
refrec **refptr_;
{
  Free(*refptr_);
}
...
/*--------------- LIST ROUTINES ---------------------------*/
Void linkname(name, line, col, list)
stryng name;
int line, col;
namelistrec **list;
{
...
Void addtonamelist(inputtoken, line, col)
stryng inputtoken;
int line, col;
{
  /*action 3*/
